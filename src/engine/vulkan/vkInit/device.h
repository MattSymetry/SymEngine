	...
	vk::PhysicalDevice choose_physical_device(const vk::Instance& instance) {
		...
		*/
		...
		for (vk::PhysicalDevice device : availableDevices) {
			...
			if (isSuitable(device) && deviceProperties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) {
				selectedDevice = device;
				break; // Stop searching once we've found a dedicated GPU
			}
			else if (isSuitable(device)) {
				selectedDevice = device;
			}
